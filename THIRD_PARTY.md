...
| [geogram](https://github.com/BrunoLevy/geogram) *(optional, on by default)* | Exact mesh-arrangement geometry kernel — linked into `geogram.so` (FetchContent, pinned to v1.10.0, statically linked). Excluded with `-DSRAVA_MODULE_GEOGRAM=OFF` | BSD-3-Clause |
| [OpenVDB](https://github.com/AcademySoftwareFoundation/openvdb) *(optional, on by default)* | Sparse volume / level-set geometry kernel — linked into `openvdb.so` (FetchContent, pinned to v12.1.1, statically linked). Excluded with `-DSRAVA_MODULE_OPENVDB=OFF` | Apache-2.0 |
| [oneTBB](https://github.com/uxlfoundation/oneTBB) *(optional, on by default)* | Task scheduler **required** by both OpenVDB and OCCT — detected from the system with `find_package(TBB CONFIG)` (Debian `libtbb-dev`) | Apache-2.0 |
| [Open CASCADE Technology](https://github.com/Open-Cascade-SAS/OCCT) *(optional, on by default)* | B-rep geometry kernel with analytic/NURBS surfaces — detected from the system with `find_package` (Debian `libocct-*`, 7.8.x). Excluded with `-DSRAVA_MODULE_OCCT=OFF` | LGPL-2.1 **with the Open CASCADE exception** |
| [Interactive and Robust Mesh Booleans](https://github.com/gcherchi/InteractiveAndRobustMeshBooleans) *(optional, on by default)* | Exact mesh-arrangement Boolean kernel using **indirect predicates** — header-only, compiled into `cherchi.so` (FetchContent, pinned to commit `7bd6c26`). Excluded with `-DSRAVA_MODULE_CHERCHI=OFF`. Pulls in its submodules: [arrangements](https://github.com/gcherchi/FastAndRobustMeshArrangements) (MIT), [Cinolib](https://github.com/mlivesu/cinolib) (MIT, which itself bundles Eigen — MPL-2.0), [Indirect_Predicates](https://github.com/MarcoAttene/Indirect_Predicates) (**LGPL-2.1**), abseil-cpp / parallel-hashmap (Apache-2.0). Its bundled oneTBB copy is **not** built — TBB comes from the system, like OpenVDB's and OCCT's. | MIT |
...
| [jemalloc](https://github.com/jemalloc/jemalloc) *(optional, on by default on Linux)* | malloc implementation linked into the two executables (`srava` / `srava_agent`) — **not** into the modules. Detected from the system (Debian `libjemalloc-dev`); falls back to the system malloc when absent. Selected with `-DSRAVA_ALLOCATOR=auto\|jemalloc\|system` | BSD-2-Clause |
