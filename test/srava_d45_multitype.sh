...
#   (2) SRAVA_DBG_CONV に **2 本の異なる [CONV] 行** (mf-mesh3d->d4-mesh3d と mf-mesh3d->d5-mesh3d) が出る
...
# (2) 多型共存: 2 本の異なる [CONV] 行 (d4-mesh3d と d5-mesh3d の両方) が出ること。
CONV=$(SRAVA_AGENT="$BOGUS_AGENT" SRAVA_DBG_CONV=1 SRAVA_CACHE_DIR="$CACHE/b" SRAVA_SOURCE="$SRC" \
       "$SRAVA" 2>&1 | grep '^\[CONV\]' | sort -u)
N=$(printf '%s\n' "$CONV" | grep -c '^\[CONV\]')
if [ "$N" != "2" ] \
   || ! printf '%s\n' "$CONV" | grep -q 'mf-mesh3d -> d4-mesh3d' \
   || ! printf '%s\n' "$CONV" | grep -q 'mf-mesh3d -> d5-mesh3d'; then
