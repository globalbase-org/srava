...
#define SRAVA_MODULE_ABI 24   /* 記述子 ABI 版。dlopen 時に不一致なら拒否。
                              ...
                              *     ★記述子の形が同じでも、共有する基底のレイアウトが変わったら版を上げる。
                              * v20: pigOpEntry に nreq (必須引数個数) 追加。
                              v3: codecs 追加 (Phase 4③')。
                              ...
                              *     ★レイアウト変更につき版必須。
                              * v19: **配線の 1 スロットを候補列へ** (#3469・2026-09-01)。pigOpWiring.want が
                              *     「スロットごとに 1 クラス」から「スロットごとに 0 終端の候補列」になった。
                              *     export_vox が「メッシュ **または** ボリューム格子」を受けるようになり、
                              *     1 クラスでは表現できなくなったため。★ポインタのサイズは変わらないので
                              *     版を上げないと**古いモジュールが静かに誤動作する**。
                              * v18: **cache_version 追加** (#3466・ひさ裁定 2026-09-01)。キャッシュキーの弁別ソルトを
                              *     「モジュール名 + **記述子の版番号**」にする。v17 は .so の size/mtime を使ったが、
                              *     ★ **強すぎた** — 結果ハッシュがビルドのたびに変わり、可換 op の引数ソート
                              *     (結果ハッシュ順) を通じて **n 項の木の形がビルド依存**になった
                              *     (実測: touch cgal.so だけで 3 box union が 27v50 ⇄ 30v56)。
                              *     名前だけでは逆に弱すぎる (計算を変えても古い結果が当たる)。
                              *     ⇒ info.txt の SRAVA_CACHE_FORMAT と同じ **手で上げる版番号**にする。
                              *     自動検出は agent バイナリの指紋 (info.txt) が引き続き担うので、
                              *     通常の再ビルドでは今までどおり全掃される。
                              * v17: **hash_salt を撤去** (#3466・ひさ裁定 2026-08-31)。キャッシュキーの弁別ソルトは
                              *     記述子の申告をやめ、レジストリが **モジュール名 + その .so の指紋 (size/mtime)** から
                              *     作る。名前の一意性はローダが既に強制しており (register_module(name) が name→id・
                              *     同名 .so は shadow)、一意な識別子を 2 つ維持していた。★「基準カーネルはソルト無し」
                              *     という例外も撤廃 — cgal が 0 だったのはモジュール機構より前のキャッシュを
                              *     無効にしないための据え置きで、設計上の基準ではない。★レイアウト変更につき版必須。
...
struct srava_module_descriptor {
	...
	/* ★ 2026-08-31 (ABI v17・#3466): 旧 hash_salt を **撤去**した。キャッシュキーの弁別ソルトは
	 *   記述子が「弁別バイト列」を申告するのをやめ、レジストリが **モジュール名 + 下の cache_version**
	 *   から作る (pigModuleRegistry::cache_salt)。名前の一意性はローダが既に強制しているので、
	 *   "\x01MFM" のような写しを別に維持する理由が無かった。 */

	/* ★ v18 (#3466・ひさ裁定 2026-09-01): **このモジュールが出す結果の版**。
	 *   キャッシュキーのソルトに入る。**計算を変えたら手で上げる** (info.txt の
	 *   SRAVA_CACHE_FORMAT と同じ運用)。0 と 1 は同じ意味 (未設定 = 1)。
	 *
	 *   ★ なぜ .so の size/mtime にしないか: **強すぎる**。結果ハッシュがビルドのたびに変わり、
	 *     .so を建て直すだけで全キャッシュが外れる。
	 *     ⚠ v18 を決めた当時はもう 1 つ「可換 op の引数ソート (結果ハッシュ順) を通じて
	 *       n 項の木の形までビルド依存になる」という理由があったが、**#3500 でそのソート自体を
	 *       撤去した**ので、いま残っているのは上のキャッシュ失効の理由だけ。
	 *   ★ なぜ名前だけにしないか: **弱すぎる**。そのモジュールの計算を変えても古い結果が当たる。
	 *   ⇒ 中間を取り、**明示的に上げる版番号**にする。上げ忘れは素通りするが、そこは都度管理。
	 *     ⚠ 通常の `cmake --build` では agent も建て直るので、info.txt の agent 指紋が
	 *       キャッシュを全掃する。この版番号が効くのは **.so だけ差し替える**場面。 */
	int               cache_version;
