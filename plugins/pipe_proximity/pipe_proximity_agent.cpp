/*
 * pipe_proximity_agent — cgal-processor プラグイン規約の最初の実プラグイン(エージェント main)。
 *   pipe_proximity(別 repo・MIT・CGAL 非依存)の自己接近検出を srava から呼べるようにする。
 *
 * この TU は SDK(pigData/::pipe)だけを include し、pipe ヘッダ(namespace pipe)は触らない
...
static std::vector<double> parse_ctrl(sPtr<pigData> a0, int *npts) {
	*npts = 0;
	...
	sPtr<pigDataArray> pts = sPtr<pigDataArray>::d_cast(a0);
...
static PPAdjustParams parse_params(sPtr<pigData> a) {
	...
	sPtr<pigDataHash> h = sPtr<pigDataHash>::d_cast(a);
	...
	if ( (v = hget(h,"fixed"))   != thNULL ) {
		sPtr<pigDataArray> fa = sPtr<pigDataArray>::d_cast(v);
		if ( fa.is_notNull() )
			for ( int i = 0 ; i < fa->length() ; ++i )
				p.fixed.push_back((int)fa->get_ix(thNEW(pigDataInteger,((INTEGER64)i)))->get_int());
	}
	if ( (v = hget(h,"pins"))    != thNULL ) {
		sPtr<pigDataArray> pa = sPtr<pigDataArray>::d_cast(v);
		if ( pa.is_notNull() )
			for ( int i = 0 ; i < pa->length() ; ++i ) {
				sPtr<pigDataHash> ph = sPtr<pigDataHash>::d_cast(
				    pa->get_ix(thNEW(pigDataInteger,((INTEGER64)i))));
...
compute_proximity(const char *op, sArray<sPtr<pigData> >& args)
{
	...
	/* 1) ctrl_pts → 平坦 [x,y,z,...] ---- */
	sPtr<pigDataArray> pts = sPtr<pigDataArray>::d_cast(args[0]);
...
compute_adjust(sArray<sPtr<pigData> >& args)
{
	...
	sPtr<pigDataHash> h2 = sPtr<pigDataHash>::d_cast(a2);
	PPAdjustParams P;
	if ( h2.is_notNull() ) {
		P = parse_params(a2);
	} else {
...
parse_bodies(sPtr<pigData> a, const char *opname, sPtr<pigData> *e) {
	...
	sPtr<pigDataArray> ba = sPtr<pigDataArray>::d_cast(a);
...
/* op で分岐(同一 bin が検出/調整/シーン/サンプルの各 op を serve)。 */
static sPtr<pigData>
compute(const char *op, sArray<sPtr<pigData> >& args)
{
...
main(void)
{
	return pigplugin::serve(&compute);
}
