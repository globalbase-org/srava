...
// eager-load は廃止)。
//
// ★★ **原則は「実際に使うモジュールに絞る」** — module("manifold.so",{}) のように
//   要るものだけを名指し、候補列 (use [...]) を書く。モジュールは増え続けているので、
//   一式ロードは常駐と起動の固定費が増え続ける。それ以上に効くのは **配線**で、
//   名指さずに全部載せると *その op がどの .so に当たるかがロード順と priority 任せ* になり、
//   **書いていないモジュールへ黙って配線されたまま値が返る**。
//
// ⇒ このファイルは **移行中・探索中の便宜品**である。旧来 (#3452 以前) の「起動時に全カーネルが
//   使える」挙動に近い状態が要るとき、あるいは「どの .so が要るのか」をこれから見極めるときに使う。
//   見極めがついたら module() の列へ置き換える (落ちた op を 1 本ずつ足せば、それが実際の依存)。
//
...
//   使う .so が決まっているなら all.sra ではなく個別に module() する (原則。上の ★★ を参照)。
