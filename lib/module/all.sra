...
//   掲載の全 20 本のうち、**nef_snc.so と デモ／テスト (demo/d2/d3/d4/d5) を除く 14 本**。
//   デモ／テストの 5 本は module/demo.sra に分けてある (include "module/demo.sra"; で読める)。
//   nef_snc.so はどちらにも入れない — 実カーネルの変種で既定でビルドもされないため、
...
module("nef_hybrid.so",     {optional:1});   // nef_snc.so は明示ロード (既定でビルドされない)
