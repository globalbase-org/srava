...
// 使い方:
//
//     include "module/all.sra";        // ← .so のロードは **こちらの仕事**
...
//
// ⚠⚠ **このファイルは `use` を書かない**。既定として候補列を敷くと
//   `module(so,{priority:N})` という別軸の指定が黙って効かなくなる (#3555 の教訓)。
//   候補列を決めるのは常に利用者。
//
// ⚠ **.so はロードしない**。ロードは all.sra / module() の仕事で、ここは値を定義するだけ。
