...
static int yyGrowStack(yyParser *p){
  ...
  if( p->yystack==&p->yystk0 ){
    ...
    if( pNew ) {
      memset(pNew,0,newSize*sizeof(pNew[0]));
      pNew[0] = p->yystk0;
    }
  }else{
    pNew = realloc(p->yystack, newSize*sizeof(pNew[0]));
    if ( pNew )
      memset(&pNew[p->yystksz],0,(newSize-p->yystksz)*sizeof(pNew[0]));
  }
  if( pNew ){
    p->yystack = pNew;
...
static void yy_shift(
  ...
  REF_SET(yytos->minor.yy0,yyMinor);
...
static void yy_reduce(
  ...
  ** enough on the stack to push the LHS value */
  ...
  //  memset(&yylhsminor,0,sizeof(yylhsminor));
  switch( yyruleno ){
...
void Parse(
  ...
  do{
    ...
    if( yyact >= YY_MIN_REDUCE ){
      ...
      yypParser->yytos--;
      yy_accept(yypParser);
      REF_SET(yyminorunion.yy0,0);
      return;
    }else{
      assert( yyact == YY_ERROR_ACTION );
      REF_SET(yyminorunion.yy0,yyminor);
#ifdef YYERRORSYMBOL
      int yymx;
#endif
#ifndef NDEBUG
