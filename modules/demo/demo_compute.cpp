...
demo_compute(const char *op, sArray<sPtr<pigData> >& args)
{
	...
	/* ★ #3554 段1 の検定台。**行名がそのまま届く**ことがここで分かる — 届いていなければ
	 *   agent の lookup_op が "unknown op: demo_pick" で落ちる (黙って基底行に落ちたりしない)。 */
	if ( op && ::strcmp(op, "demo_pick#a") == 0 ) return thNEW(pigDataInteger,((INTEGER64)1));
	if ( op && ::strcmp(op, "demo_pick#b") == 0 ) return thNEW(pigDataInteger,((INTEGER64)2));
	if ( op && ::strcmp(op, "demo_only#b") == 0 ) return thNEW(pigDataInteger,((INTEGER64)2));
	if ( op && ::strcmp(op, "demo_xy#flat") == 0 ) return thNEW(pigDataInteger,((INTEGER64)1));
	if ( op && ::strcmp(op, "demo_xy#any")  == 0 ) return thNEW(pigDataInteger,((INTEGER64)2));
