...
## 並列プリミティブ (`src/cd_parallel.cpp`) — 常駐プール (akira-project #3419)

`parallel_for` は**呼び出しスレッドごとの常駐ワーカープール**で動く。以前は呼び出しのたびに
`std::thread` を生成して join していた (spawn-per-call) が、座標降下のホットループ底から
呼ばれるため、in-proc (既定 `EXEC_THREAD`) では planner プロセス内でスレッドの生成・破棄が
絶え間なく起きていた。害は速度だけではなく、tid を基準にした観測を壊すこと
(`753bf8b` の原因)、他プロセスとの mm ロック競合、中断点の不在。

改修時に守ること:

- **プールは thread_local** (プロセス共有にしない)。in-proc では複数の agent スレッドが同時に
  `parallel_for` へ入るので、共有プールにすると op 間で直列化してしまう。
- **join は所有スレッドの終了時のみ** (`thread_local` のデストラクタ)。**静的デストラクタで
  join しない** — teardown デッドロックの前科がある。ワーカーは自前の条件変数でしか待たないので、
  停止フラグを立てて起こせば join は即返る。
- **呼び出し元も 1 本ぶんの仕事をする**。ワーカーは `T-1` 本。
- **添字はちょうど 1 回ずつ実行する** (atomic の work-steal)。決定性はこの性質に依存している。
- `fn` が投げたら最初の例外を**呼び出し元へ送り直す**。ワーカー上の throw をそのまま逃がすと
  `std::terminate` になり、in-proc では planner ごと落ちる。
