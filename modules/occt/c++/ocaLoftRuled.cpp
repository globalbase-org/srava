/*
 * ocaLoftRuled — loft_ruled(断面, 断面, …) (#3511)。断面間を **直線で結ぶ** (線織面)。
 *
...
#include	<stdio.h>
#include	"_ts2/c++/ocaLoftRuled_.h"

CLASS_TINYSTATE(oc/c++/ocaLoftRuled,pig/c++/ptsCalcBody)
...
ocaLoftRuled_::ocaLoftRuled_(TS_ARGS0)
...
void
ocaLoftRuled_::compute()
{
	ocShape::ensure_init();   /* ★ OCCT の診断出力を stdout から外す */
	const char *msg = 0;
	char why[512];
	why[0] = '\0';
	out = ocShape::loft_from_args(args, true, &msg, why, (int)sizeof why);
	if ( ! out.is_notNull() ) {
		char b[600];
