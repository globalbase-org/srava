...
ocaChamfer_::compute()
{
	...
	if ( ! in.is_notNull() ) {
		result = oca_err(thNEW(stdString,("chamfer: needs an OCCT shape")));
		return;
	}
	double d = ( na > 1 ) ? (*args)[1]->get_flt() : 0.0;
	if ( d < 0.0 ) {
		result = oca_err(thNEW(stdString,("chamfer: distance must be >= 0")));
		return;
	}
	...
	char why[512];
	why[0] = '\0';
out = in->op_chamfer(d, why, (int)sizeof why, &brk_);
	if ( ! out.is_notNull() ) {
		if ( (result = oc_abort_err(brk_, "chamfer")) != thNULL ) return;   /* ★ #3498 */
