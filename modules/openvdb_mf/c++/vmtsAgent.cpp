/*
 * vmtsAgent — ★ **openvdb ⇄ manifold の境界モジュール** (#3434・2026-08-22)。
 *
 ...
 *
 * ライセンス: OpenVDB (Apache-2.0) + Manifold (Apache-2.0)。**GPL は混ざらない**。
 */
...
#include	"vm/c++/vmaVoxelize.h"
#include	"vm/c++/vmaIsosurface.h"
#include	"_ts2/c++/vmtsAgent_.h"

#include	<stdio.h>    /* SRAVA_LOAD_LOG の診断出力 */
#include	<stdlib.h>   /* getenv */

CLASS_TINYSTATE(vm/c++/vmtsAgent,pig/c++/ptsGenericAgent)
...
static const pigOpEntry OPS[] = {
	...
	{ "voxelize",   VOXELIZE_IN, 2, AK_CACHE, OPWIRE(vmaVoxelize, mfGeom),   0, "(mf-mesh3d)->" VD_TYPE },
	{ "isosurface", GRIDISO_IN,  2, AK_CACHE, OPWIRE(vmaIsosurface, vdGeom), 0, "(" VD_TYPE ")->mf-mesh3d" },
};
...
vmtsAgent_::vmtsAgent_(TS_ARGS0)
        : ptsGenericAgent_(parent)
{
    TS_CPARGS0
}

const pigOpEntry*	vmtsAgent_::agent_ops()	  { return OPS; }
int			vmtsAgent_::agent_n_ops() { return N_OPS; }
const char*		vmtsAgent_::agent_name()  { return "openvdb_mf"; }


static sPtr<ptsAgent>
mk_vmtsAgent(sPtr<ptsObject> parent)
{
	return sPtr<ptsAgent>::d_cast(thNEW(vmtsAgent,(parent)));
}
...
extern const srava_module_descriptor vmtsAgent_descriptor;
extern const srava_module_descriptor vmtsAgent_descriptor = {
