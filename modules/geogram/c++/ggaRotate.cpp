/*
 * ggaRotate — rotate(m, axis, deg) — 原点通過の軸まわりの回転。axis は "x"/"y"/"z" または [x,y,z]。
 *
...
#include	"_ts2/c++/ggaRotate_.h"

CLASS_TINYSTATE(gg/c++/ggaRotate,pig/c++/ptsCalcBody)
...
ggaRotate_::ggaRotate_(TS_ARGS0)
...
ggaRotate_::compute()
{
	...
	if ( ! in.is_notNull() ) {
		result = gga_err(thNEW(stdString,("rotate: needs a geogram mesh")));
		return;
	}
