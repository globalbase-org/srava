/*
 * ggaBbox — bbox(m) — 軸平行 AABB を [min 隅, max 隅] で返す。
 *
...
#include	"_ts2/c++/ggaBbox_.h"

CLASS_TINYSTATE(gg/c++/ggaBbox,pig/c++/ptsCalcBody)
...
ggaBbox_::compute()
{
	...
	if ( ! in.is_notNull() ) {
		result = gga_err(thNEW(stdString,("bbox: needs a geogram mesh")));
		return;
	}
