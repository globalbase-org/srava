/*
 * ggaPyramid — pyramid(n, h, r) — 正 n 角錐 (底面は z=0 の XY 平面・頂点は z=h) の計算本体 (geogram 版・#3474)。
...
#include	"_ts2/c++/ggaPyramid_.h"

CLASS_TINYSTATE(gg/c++/ggaPyramid,pig/c++/ptsCalcBody)
...
ggaPyramid_::ggaPyramid_(TS_ARGS0)
...
ggaPyramid_::compute()
{
	...
	if ( srava_geo::check_sides(n, &n) != srava_geo::SEGS_OK ) {   /* ★ #3530: n は形そのもの = 既定値なし */
		result = gga_err(thNEW(stdString,(srava_geo::sides_error("pyramid").c_str()))); return; }
	if ( !(h > 0) ) { result = gga_err(thNEW(stdString,("pyramid: height must be > 0"))); return; }
	if ( !(r > 0) ) { result = gga_err(thNEW(stdString,("pyramid: radius must be > 0"))); return; }
