/*
 * ggaScale — scale(m, s | [sx,sy,sz]) — 原点中心の拡大縮小。負の倍率は反射になる。
 *
...
#include	"_ts2/c++/ggaScale_.h"

CLASS_TINYSTATE(gg/c++/ggaScale,pig/c++/ptsCalcBody)
...
ggaScale_::compute()
{
	...
	if ( ! in.is_notNull() ) {
		result = gga_err(thNEW(stdString,("scale: needs a geogram mesh")));
		return;
	}
