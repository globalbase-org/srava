/*
 * ggaTube — tube(path[, segs]) — 折れ線まわりの掃引管 (geogram 版・#3474)。
...
ggaTube_::compute()
{
	...
	if ( nraw < 2 ) {
		result = gga_err(thNEW(stdString,("tube: needs >= 2 path vertices ([[[x,y,z],r],...])")));
		return;
	}
	...
	for ( int i = 0 ; i < nraw ; ++i ) {
		...
		if ( ! pr.is_notNull() || pr->length() < 2 ) {
			result = gga_err(thNEW(stdString,("tube: each vertex must be [pos, r] (pos=[x,y,z])")));
			return;
		}
		...
		if ( pl < 3 ) {
			result = gga_err(thNEW(stdString,("tube: this kernel has no 2D type, so the path must be 3D ([[x,y,z],r]); use \"cgal\"::tube or \"manifold\"::tube for 2D ribbons")));
			return;
		}
		...
		if ( Rraw[(size_t)i] < 0.0 ) {
			result = gga_err(thNEW(stdString,("tube: radius must be >= 0")));
			return;
		}
	}
	...
	if ( P.size() < 2 ) {
		result = gga_err(thNEW(stdString,("tube: needs >= 2 distinct path vertices (all given vertices coincide)")));
		return;
	}
	...
	if ( st == srava_geo::TUBE_ERR_ZERO_SEGMENT ) {
		result = gga_err(thNEW(stdString,("tube: two consecutive zero-radius vertices (degenerate segment)")));
		return;
	}
	if ( st != srava_geo::TUBE_OK ) {
		result = gga_err(thNEW(stdString,("tube: duplicate consecutive path vertices")));
		return;
	}
	mesh = sink.finish();
}
