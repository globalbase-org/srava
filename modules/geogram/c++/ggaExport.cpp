/*
 * ggaExport — export(path, mesh, unit) の計算本体 (geogram 版・nfaExport のミラー)。結果は D_REF。
 * 書き出しは GEO::mesh_save の拡張子ディスパッチ (off/stl/obj/ply)。単位付き形式 (3MF/AMF) は非対応。
 */
...
static pHashKeyType gg_hash_file(const char *path)
{
...
#include	"_ts2/c++/ggaExport_.h"

CLASS_TINYSTATE(gg/c++/ggaExport,pig/c++/ptsCalcBody)
...
ggaExport_::ggaExport_(TS_ARGS0)
