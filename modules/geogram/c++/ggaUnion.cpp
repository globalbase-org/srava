/*
 * ggaUnion — union(a,b) の計算本体 (geogram 版)。mesh arrangement + 厳密述語による二項ブール。
 */
...
#include	<stdio.h>
#include	"_ts2/c++/ggaUnion_.h"

CLASS_TINYSTATE(gg/c++/ggaUnion,pig/c++/ptsCalcBody)
...
ggaUnion_::compute()
{
	...
	char why[512];
	why[0] = '\0';
	mesh = ggMesh::bool_from_args(args, "union", &msg, why, (int)sizeof why);
	if ( ! mesh.is_notNull() ) {
