/*
 * ggaMirror — mirror(m, axis | [nx,ny,nz]) — 原点を通る平面での鏡像。det=-1 の反射。
 *
...
#include	"_ts2/c++/ggaMirror_.h"

CLASS_TINYSTATE(gg/c++/ggaMirror,pig/c++/ptsCalcBody)
...
ggaMirror_::ggaMirror_(TS_ARGS0)
...
ggaMirror_::compute()
{
	...
	if ( ! in.is_notNull() ) {
		result = gga_err(thNEW(stdString,("mirror: needs a geogram mesh")));
		return;
	}
