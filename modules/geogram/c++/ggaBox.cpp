/*
 * ggaBox — box(w,h,d) / boxa([w,h,d]) の計算本体 (geogram 版)。cgal/manifold/nef と同じ軸平行の箱 (0,0,0)-(w,h,d)。
 */
...
#include	"_ts2/c++/ggaBox_.h"

CLASS_TINYSTATE(gg/c++/ggaBox,pig/c++/ptsCalcBody)
...
ggaBox_::compute()
{
	...
	if ( !(w > 0) || !(h > 0) || !(d > 0) ) {
		result = gga_err(thNEW(stdString,("box: sizes must be > 0")));
		return;
	}
	mesh = thNEW(ggMesh,());
