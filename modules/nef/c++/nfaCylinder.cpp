/*
 * nfaCylinder — cylinder(r, h, seg) — 円柱 (原点中心・軸 +Z) の計算本体 (nef 版・#3474)。
...
#include	"_ts2/c++/nfaCylinder_.h"

CLASS_TINYSTATE(nf/c++/nfaCylinder,pig/c++/ptsCalcBody)
...
nfaCylinder_::nfaCylinder_(TS_ARGS0)
...
nfaCylinder_::compute()
{
	...
	if ( srava_geo::check_segs(seg_in, 0, &seg) != srava_geo::SEGS_OK ) {
		result = nfa_err(thNEW(stdString,(srava_geo::segs_error("cylinder").c_str()))); return;
	}
	if ( !(r > 0) ) { result = nfa_err(thNEW(stdString,("cylinder: radius must be > 0"))); return; }
	if ( !(h > 0) ) { result = nfa_err(thNEW(stdString,("cylinder: height must be > 0"))); return; }
