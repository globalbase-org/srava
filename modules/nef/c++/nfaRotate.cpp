/*
 * nfaRotate — rotate(m, axis, deg) — 原点通過の軸まわりの回転。axis は "x"/"y"/"z" または [x,y,z]。
 *
...
#include	"_ts2/c++/nfaRotate_.h"

CLASS_TINYSTATE(nf/c++/nfaRotate,pig/c++/ptsCalcBody)
...
nfaRotate_::nfaRotate_(TS_ARGS0)
...
nfaRotate_::compute()
{
	...
	if ( ! in.is_notNull() ) {
		result = nfa_err(thNEW(stdString,("rotate: needs a Nef mesh")));
		return;
	}
