/*
 * nfaTube — tube_ruled(path[, segs]) — 折れ線まわりの掃引管 (nef 版・#3474)。
...
nfaTube_::compute()
{
	...
	if ( srava_geo::check_segs(segs_in, 32, &segs) != srava_geo::SEGS_OK ) {
		result = nfa_err(thNEW(stdString,(srava_geo::segs_error("tube_ruled").c_str()))); return;
	}
	int nraw = path.is_notNull() ? path->length() : 0;
	if ( nraw < 2 ) {
		result = nfa_err(thNEW(stdString,("tube_ruled: needs >= 2 path vertices ([[[x,y,z],r],...])")));
		return;
	}
	...
	for ( int i = 0 ; i < nraw ; ++i ) {
		...
		if ( ! pr.is_notNull() || pr->length() < 2 ) {
			result = nfa_err(thNEW(stdString,("tube_ruled: each vertex must be [pos, r] (pos=[x,y,z])")));
			return;
		}
		...
		if ( pl < 3 ) {
			result = nfa_err(thNEW(stdString,("tube_ruled: this kernel has no 2D type, so the path must be 3D ([[x,y,z],r]); use \"cgal\"::tube or \"manifold\"::tube for 2D ribbons")));
			return;
		}
		...
		if ( Rraw[(size_t)i] < 0.0 ) {
			result = nfa_err(thNEW(stdString,("tube_ruled: radius must be >= 0")));
			return;
		}
	}
	...
	if ( P.size() < 2 ) {
		result = nfa_err(thNEW(stdString,("tube_ruled: needs >= 2 distinct path vertices (all given vertices coincide)")));
		return;
	}
	...
	if ( st == srava_geo::TUBE_ERR_ZERO_SEGMENT ) {
		result = nfa_err(thNEW(stdString,("tube_ruled: two consecutive zero-radius vertices (degenerate segment)")));
		return;
	}
	if ( st != srava_geo::TUBE_OK ) {
		result = nfa_err(thNEW(stdString,("tube_ruled: duplicate consecutive path vertices")));
		return;
	}
	mesh = sink.finish();
}
