...
#include	"vd/c++/vdGrid.h"          /* ★ #3469: vd-grid3d 入力 */
...
vcaExportVox_::compute()
{
	...
	for ( int i = 0 ; i < nmesh ; ++i ) {
		...
		if ( m3.is_notNull() ) { mesh_to_tris(m3->mesh(), regionTris[i], lo, hi); continue; }
		sPtr<vdGrid> vg = sPtr<vdGrid>::d_cast((*args)[2+i]);
		if ( vg.is_notNull() && vg->grid() ) {
			regionGrid[i] = vg->grid();
			/* bbox は **active voxel** の世界座標。level set の active は narrow band だが、
			 * 帯は表面を覆うので物体の外接箱としてはこれで足りる (内部は inactive タイル)。 */
			openvdb::CoordBBox ib = vg->grid()->evalActiveVoxelBoundingBox();
			...
			for ( int c = 0 ; c < 8 ; ++c ) {
				...
				openvdb::Vec3d w = vg->grid()->transform().indexToWorld(k.asVec3d());
				for ( int a = 0 ; a < 3 ; ++a ) {
