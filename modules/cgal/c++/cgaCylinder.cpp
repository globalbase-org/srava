/*
 * cgaCylinder — cylinder(r, h, seg) — 円柱 (原点中心・軸 +Z) の計算本体 (cgal 版・#3474)。
...
#include	"_ts2/c++/cgaCylinder_.h"

CLASS_TINYSTATE(cg/c++/cgaCylinder,pig/c++/ptsCalcBody)
...
cgaCylinder_::cgaCylinder_(TS_ARGS0)
...
cgaCylinder_::compute()
{
	...
	if ( srava_geo::check_segs(seg_in, 0, &seg) != srava_geo::SEGS_OK ) {
		result = cga_err(thNEW(stdString,(srava_geo::segs_error("cylinder").c_str()))); return;
	}
	if ( !(r > 0) ) { result = cga_err(thNEW(stdString,("cylinder: radius must be > 0"))); return; }
	if ( !(h > 0) ) { result = cga_err(thNEW(stdString,("cylinder: height must be > 0"))); return; }
