 ...
 * 保持する。中間 blob は作らない。cgatsAgent は get_result() で受けた cgMesh を set_body 経由の WriterMesh に(#3406 2026-07-30: get_body 統合)
...
protected:
	...
	sPtr<cgMesh3D>	mesh;     /* compute() が作った箱(get_result が agent へ返す) */
private:
	TS_DEFARGS
};
...
/* w,h,d の箱(原点隅)を CGAL で cgMesh に作る。シリアライズは writer 側(保存は agent の set_body 経由)。
