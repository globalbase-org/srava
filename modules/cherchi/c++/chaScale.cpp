/*
 * chaScale — scale(m, s | [sx,sy,sz]) — 原点中心の拡大縮小。負の倍率は反射になる。
 *
...
#include	"_ts2/c++/chaScale_.h"

CLASS_TINYSTATE(ch/c++/chaScale,pig/c++/ptsCalcBody)
...
chaScale_::compute()
{
	...
	if ( ! in.is_notNull() ) {
		result = cha_err(thNEW(stdString,("scale: needs a cherchi mesh")));
		return;
	}
