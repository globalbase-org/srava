/*
 * chaMirror — mirror(m, axis | [nx,ny,nz]) — 原点を通る平面での鏡像。det=-1 の反射。
 *
...
#include	"_ts2/c++/chaMirror_.h"

CLASS_TINYSTATE(ch/c++/chaMirror,pig/c++/ptsCalcBody)
...
chaMirror_::chaMirror_(TS_ARGS0)
...
chaMirror_::compute()
{
	...
	if ( ! in.is_notNull() ) {
		result = cha_err(thNEW(stdString,("mirror: needs a cherchi mesh")));
		return;
	}
