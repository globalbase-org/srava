/*
 * chaCone — cone(r, h, seg) — 円錐 (原点中心・軸 +Z・底面 z=-h/2・頂点 z=+h/2) の計算本体 (cherchi 版・#3474)。
...
#include	"_ts2/c++/chaCone_.h"

CLASS_TINYSTATE(ch/c++/chaCone,pig/c++/ptsCalcBody)
...
chaCone_::compute()
{
	...
	if ( srava_geo::check_segs(seg_in, 0, &seg) != srava_geo::SEGS_OK ) {
		result = cha_err(thNEW(stdString,(srava_geo::segs_error("cone").c_str()))); return;
	}
	if ( !(r > 0) ) { result = cha_err(thNEW(stdString,("cone: radius must be > 0"))); return; }
	if ( !(h > 0) ) { result = cha_err(thNEW(stdString,("cone: height must be > 0"))); return; }
