/*
 * chaCylinder — cylinder(r, h, seg) — 円柱 (原点中心・軸 +Z) の計算本体 (cherchi 版・#3474)。
...
#include	"_ts2/c++/chaCylinder_.h"

CLASS_TINYSTATE(ch/c++/chaCylinder,pig/c++/ptsCalcBody)
...
chaCylinder_::chaCylinder_(TS_ARGS0)
...
chaCylinder_::compute()
{
	...
	if ( srava_geo::check_segs(seg_in, 0, &seg) != srava_geo::SEGS_OK ) {
		result = cha_err(thNEW(stdString,(srava_geo::segs_error("cylinder").c_str()))); return;
	}
	if ( !(r > 0) ) { result = cha_err(thNEW(stdString,("cylinder: radius must be > 0"))); return; }
	if ( !(h > 0) ) { result = cha_err(thNEW(stdString,("cylinder: height must be > 0"))); return; }
