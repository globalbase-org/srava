/*
 * d4Mesh — 第3(mesh 出力)カーネルの幾何本体実装 (rev4 Phase D-3)。CGAL/Manifold 非依存。
 */
...
static void put_u32(d4ChunkSink &s, uint32_t v) {
	uint8_t b[4] = { (uint8_t)v, (uint8_t)(v>>8), (uint8_t)(v>>16), (uint8_t)(v>>24) };
	s.chunk(b, 4);
}
static void put_f64(d4ChunkSink &s, double d) {
	uint8_t b[8]; ::memcpy(b, &d, 8); s.chunk(b, 8);
}
static uint32_t get_u32(d4ChunkSource &s) {
	uint8_t b[4]; s.pull(b, 4);
	return (uint32_t)b[0] | ((uint32_t)b[1]<<8) | ((uint32_t)b[2]<<16) | ((uint32_t)b[3]<<24);
}
static double get_f64(d4ChunkSource &s) {
...
{
	char b[64];
	::snprintf(b, sizeof b, "d4mesh(%u,%u)", (unsigned)nv(), (unsigned)nf());
	return thNEW(stdString,(b));
}

void
d4Mesh::encode(d4ChunkSink &sink)
{
	put_u32(sink, nv());
	put_u32(sink, nf());
...
void
d4Mesh::decode(d4ChunkSource &src)
{
...
sPtr<d4Mesh>
d4Mesh::cube(double s)
{
	sPtr<d4Mesh> m = thNEW(d4Mesh,());
...
sPtr<d4Mesh>
d4Mesh::merge(sPtr<d4Mesh> a, sPtr<d4Mesh> b)
{
	if ( ! a.is_notNull() || ! b.is_notNull() ) return sPtr<d4Mesh>();
	sPtr<d4Mesh> m = thNEW(d4Mesh,());
	uint32_t offv = a->nv();
	m->verts_ = a->verts_;
...
sPtr<d4Mesh>
d4Mesh::create_for_meta(const uint8_t *meta, int len)
{
	/* 自ネイティブ D4M3 に加え、Manifold の MFM3 も受理する (⑤ cross-module 変換・P4)。
	 * mfMesh の MFM3 codec framing は d4Mesh と完全同一 ([u32 nv][u32 nf] verts(f64) faces(u32)・
	 * little-endian) なので、同じ decode() で読める = MFM3→d4-mesh3d の cross reader が成立する。
	 * どちらのタグでも生成するのは d4Mesh (=自型)。reader_for が out_type=d4-mesh3d で選ぶので、
	 * MFM3 を「自型で消費したい」要求のときだけこの reader が呼ばれる (mf 自身の読みには影響しない)。 */
	if ( len == 4 && ( ::memcmp(meta, "D4M3", 4) == 0 || ::memcmp(meta, "MFM3", 4) == 0 ) )
		return thNEW(d4Mesh,());
	return sPtr<d4Mesh>();
}
