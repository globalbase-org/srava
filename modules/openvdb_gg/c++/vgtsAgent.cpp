/*
 * vgtsAgent — ★ **openvdb ⇄ manifold の境界モジュール** (#3434・2026-08-22)。
 *
 ...
 *
 * ライセンス: OpenVDB (Apache-2.0) + geogram (BSD-3)。**GPL は混ざらない**。
 */
...
#include	"vg/c++/vgaVoxelize.h"
#include	"vg/c++/vgaIsosurface.h"
#include	"_ts2/c++/vgtsAgent_.h"

#include	<stdio.h>    /* SRAVA_LOAD_LOG の診断出力 */
#include	<stdlib.h>   /* getenv */

CLASS_TINYSTATE(vg/c++/vgtsAgent,pig/c++/ptsGenericAgent)
...
static const pigOpEntry OPS[] = {
	...
	{ "voxelize",   VOXELIZE_IN, 2, AK_CACHE, OPWIRE(vgaVoxelize, ggGeom),   0, "(gg-mesh3d)->" VD_TYPE },
	{ "isosurface", GRIDISO_IN,  2, AK_CACHE, OPWIRE(vgaIsosurface, vdGeom), 0, "(" VD_TYPE ")->gg-mesh3d" },
};
...
vgtsAgent_::vgtsAgent_(TS_ARGS0)
        : ptsGenericAgent_(parent)
{
    TS_CPARGS0
}

const pigOpEntry*	vgtsAgent_::agent_ops()	  { return OPS; }
int			vgtsAgent_::agent_n_ops() { return N_OPS; }
const char*		vgtsAgent_::agent_name()  { return "openvdb_gg"; }


static sPtr<ptsAgent>
mk_vgtsAgent(sPtr<ptsObject> parent)
{
	return sPtr<ptsAgent>::d_cast(thNEW(vgtsAgent,(parent)));
}
...
extern const srava_module_descriptor vgtsAgent_descriptor;
extern const srava_module_descriptor vgtsAgent_descriptor = {
	...
	.provides      = openvdb_gg_provides,   /* 階層 × 型名 × 4CC (ABI v16) */
