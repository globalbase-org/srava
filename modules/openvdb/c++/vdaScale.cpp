...
#include	<string>
#include	"_ts2/c++/vdaScale_.h"

#include	<stdio.h>

CLASS_TINYSTATE(vd/c++/vdaScale,pig/c++/ptsCalcBody)
...
vdaScale_::compute()
{
	...
	if ( ! vd_arena_guard("scale", [&]{

	vdGrid::ensure_init();
	int na = ( args != 0 ) ? args->length() : 0;
	sPtr<vdGrid> in = ( na > 0 ) ? sPtr<vdGrid>::d_cast((*args)[0]) : sPtr<vdGrid>();
	if ( ! in.is_notNull() ) {
		result = vda_err(thNEW(stdString,("scale: needs an openvdb grid")));
		return;
	}
	...
	if ( ! out.is_notNull() ) {
		if ( (result = vd_abort_err(brk_, "scale")) != thNULL ) return;
		result = vda_err(thNEW(stdString,(
		    "scale: openvdb resampleToMatch failed")));
	}
	}, vdwhy) )
