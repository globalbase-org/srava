/*
 * vdaIntersection — intersection(a, b) の計算本体 (#3434 P2)。
...
CLASS_TINYSTATE(vd/c++/vdaIntersection,pig/c++/ptsCalcBody)
...
vdaIntersection_::vdaIntersection_(TS_ARGS0)
...
vdaIntersection_::compute()
{
	...
	const char *msg = 0;
	char ebuf[224];
	out = vdGrid::bool_from_args(args, "intersection", &msg, ebuf, (int)sizeof ebuf);
	if ( ! out.is_notNull() ) {
		char b[256];
