...
#include	<string>
#include	"_ts2/c++/vdaMirror_.h"

#include	<stdio.h>

CLASS_TINYSTATE(vd/c++/vdaMirror,pig/c++/ptsCalcBody)
...
vdaMirror_::vdaMirror_(TS_ARGS0)
...
vdaMirror_::compute()
{
	...
	if ( ! vd_arena_guard("mirror", [&]{

	vdGrid::ensure_init();
	int na = ( args != 0 ) ? args->length() : 0;
	sPtr<vdGrid> in = ( na > 0 ) ? sPtr<vdGrid>::d_cast((*args)[0]) : sPtr<vdGrid>();
	if ( ! in.is_notNull() ) {
		result = vda_err(thNEW(stdString,("mirror: needs an openvdb grid")));
		return;
	}
