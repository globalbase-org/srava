/*
 * vdaCone — cone(r, h, seg) — 円錐 (原点中心・軸 +Z・底面 z=-h/2・頂点 z=+h/2) の計算本体 (openvdb 版・#3474)。
...
#include	<string>
#include	"_ts2/c++/vdaCone_.h"

CLASS_TINYSTATE(vd/c++/vdaCone,pig/c++/ptsCalcBody)
...
vdaCone_::compute()
{
	...
	if ( srava_geo::check_segs(seg_in, 0, &seg) != srava_geo::SEGS_OK ) {
		result = vda_err(thNEW(stdString,(srava_geo::segs_error("cone").c_str()))); return;
	}
	double dx = ( na > 3 ) ? (*args)[3]->get_flt() : 0.0;
	if ( !(r > 0) ) { result = vda_err(thNEW(stdString,("cone: radius must be > 0"))); return; }
	if ( !(h > 0) ) { result = vda_err(thNEW(stdString,("cone: height must be > 0"))); return; }
	if ( !(dx > 0) ) { result = vda_err(thNEW(stdString,("cone: last argument is dx (voxel size in world units) and must be > 0"))); return; }

	vdTriSink sink;
	srava_geo::make_cone(r, h, seg, sink);
	out = sink.finish(dx);
	if ( ! out.is_notNull() )
		result = vda_err(thNEW(stdString,("cone: openvdb meshToLevelSet failed")));
	}, vdwhy) )
