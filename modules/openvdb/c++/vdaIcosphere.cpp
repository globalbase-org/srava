/*
 * vdaIcosphere — icosphere(r, subdiv) — 測地球 (正二十面体を細分) の計算本体 (openvdb 版・#3474)。
...
#include	<string>
#include	"_ts2/c++/vdaIcosphere_.h"

CLASS_TINYSTATE(vd/c++/vdaIcosphere,pig/c++/ptsCalcBody)
...
vdaIcosphere_::vdaIcosphere_(TS_ARGS0)
...
vdaIcosphere_::compute()
{
	...
	if ( !(r > 0) ) { result = vda_err(thNEW(stdString,("icosphere: radius must be > 0"))); return; }
	if ( !(dx > 0) ) { result = vda_err(thNEW(stdString,("icosphere: last argument is dx (voxel size in world units) and must be > 0"))); return; }

	vdTriSink sink;
	srava_geo::make_geodesic(srava_geo::SEED_ICOSAHEDRON, n, r, sink);
	out = sink.finish(dx);
	if ( ! out.is_notNull() )
		result = vda_err(thNEW(stdString,("icosphere: openvdb meshToLevelSet failed")));
	}, vdwhy) )
