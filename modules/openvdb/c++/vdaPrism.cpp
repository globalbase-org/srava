/*
 * vdaPrism — prism(n, h, r) — 正 n 角柱 (底面 z=0・天面 z=h) の計算本体 (openvdb 版・#3474)。
...
CLASS_TINYSTATE(vd/c++/vdaPrism,pig/c++/ptsCalcBody)
...
vdaPrism_::compute()
{
	...
	out = sink.finish(dx);
	if ( ! out.is_notNull() )
		result = vda_err(thNEW(stdString,("prism: openvdb meshToLevelSet failed")));
	}, vdwhy) )
