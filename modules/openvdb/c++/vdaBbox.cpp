/*
 * vdaBbox — bbox(m) — 軸平行 AABB を [min 隅, max 隅] で返す。
 *
...
#include	"_ts2/c++/vdaBbox_.h"

#include	<string>

CLASS_TINYSTATE(vd/c++/vdaBbox,pig/c++/ptsCalcBody)
...
vdaBbox_::compute()
{
	...
	if ( ! vd_arena_guard("bbox", [&]{

	vdGrid::ensure_init();
	int na = ( args != 0 ) ? args->length() : 0;
	sPtr<vdGrid> in = ( na > 0 ) ? sPtr<vdGrid>::d_cast((*args)[0]) : sPtr<vdGrid>();
	if ( ! in.is_notNull() ) {
		result = vda_err(thNEW(stdString,("bbox: needs an openvdb grid")));
		return;
	}
	double mn[3], mx[3];
	in->op_bbox(mn, mx);
