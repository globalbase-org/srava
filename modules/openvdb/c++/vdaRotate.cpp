...
#include	<string>
#include	"_ts2/c++/vdaRotate_.h"

#include	<stdio.h>

CLASS_TINYSTATE(vd/c++/vdaRotate,pig/c++/ptsCalcBody)
...
vdaRotate_::vdaRotate_(TS_ARGS0)
...
vdaRotate_::compute()
{
	...
	if ( ! vd_arena_guard("rotate", [&]{

	vdGrid::ensure_init();
	int na = ( args != 0 ) ? args->length() : 0;
	sPtr<vdGrid> in = ( na > 0 ) ? sPtr<vdGrid>::d_cast((*args)[0]) : sPtr<vdGrid>();
	if ( ! in.is_notNull() ) {
		result = vda_err(thNEW(stdString,("rotate: needs an openvdb grid")));
		return;
	}
	...
	if ( ! out.is_notNull() ) {
		if ( (result = vd_abort_err(brk_, "rotate")) != thNULL ) return;
		result = vda_err(thNEW(stdString,(
		    "rotate: openvdb resampleToMatch failed")));
	}
	}, vdwhy) )
