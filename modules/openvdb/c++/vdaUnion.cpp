/*
 * vdaUnion — union(a, b) の計算本体 (#3434 P2)。
...
#include	<string>
#include	"_ts2/c++/vdaUnion_.h"

#include	<stdio.h>

CLASS_TINYSTATE(vd/c++/vdaUnion,pig/c++/ptsCalcBody)
...
vdaUnion_::compute()
{
	...
	if ( ! vd_arena_guard("union", [&]{

	vdGrid::ensure_init();
	/* ★ #3436 P4: n 項で受ける (agent の中で逐次に畳む = 中間 .vdb の往復が消える)。 */
	const char *msg = 0;
	char ebuf[224];
	out = vdGrid::bool_from_args(args, "union", &msg, ebuf, (int)sizeof ebuf);
	if ( ! out.is_notNull() ) {
		char b[256];
		::snprintf(b, sizeof b, "union: %s", msg ? msg : "openvdb CSG failed");
