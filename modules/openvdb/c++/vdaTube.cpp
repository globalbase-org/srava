/*
 * vdaTube — tube(path[, segs]) — 折れ線まわりの掃引管 (openvdb 版・#3474)。
 ...
 * ★ 末尾は **dx (ボクセルサイズ)**。segs も dx も要るので tube(path, segs, dx)。
 */
...
vdaTube_::compute()
{
	...
	if ( !(dx > 0) ) {
		result = vda_err(thNEW(stdString,("tube: last argument is dx (voxel size in world units) and must be > 0")));
		return;
	}
	int nraw = path.is_notNull() ? path->length() : 0;
	if ( nraw < 2 ) {
		result = vda_err(thNEW(stdString,("tube: needs >= 2 path vertices ([[[x,y,z],r],...])")));
		return;
	}
	...
	for ( int i = 0 ; i < nraw ; ++i ) {
		...
		if ( ! pr.is_notNull() || pr->length() < 2 ) {
			result = vda_err(thNEW(stdString,("tube: each vertex must be [pos, r] (pos=[x,y,z])")));
			return;
		}
		...
		if ( pl < 3 ) {
			result = vda_err(thNEW(stdString,("tube: this kernel has no 2D type, so the path must be 3D ([[x,y,z],r]); use \"cgal\"::tube or \"manifold\"::tube for 2D ribbons")));
			return;
		}
		...
		if ( Rraw[(size_t)i] < 0.0 ) {
			result = vda_err(thNEW(stdString,("tube: radius must be >= 0")));
			return;
		}
	}
	...
	if ( P.size() < 2 ) {
		result = vda_err(thNEW(stdString,("tube: needs >= 2 distinct path vertices (all given vertices coincide)")));
		return;
	}
	...
	if ( st == srava_geo::TUBE_ERR_ZERO_SEGMENT ) {
		result = vda_err(thNEW(stdString,("tube: two consecutive zero-radius vertices (degenerate segment)")));
		return;
	}
	if ( st != srava_geo::TUBE_OK ) {
		result = vda_err(thNEW(stdString,("tube: duplicate consecutive path vertices")));
		return;
	}
	out = sink.finish(dx);
	if ( ! out.is_notNull() )
		result = vda_err(thNEW(stdString,("tube: openvdb meshToLevelSet failed")));
	}, vdwhy) )
