/*
 * vdaTransform — transform の計算本体 (#3463)。
...
#include	<string>
#include	"_ts2/c++/vdaTransform_.h"

#include	<stdio.h>

CLASS_TINYSTATE(vd/c++/vdaTransform,pig/c++/ptsCalcBody)
...
vdaTransform_::vdaTransform_(TS_ARGS0)
...
vdaTransform_::compute()
{
	...
	if ( ! out.is_notNull() ) {
		if ( (result = vd_abort_err(brk_, "transform")) != thNULL ) return;
		result = vda_err(thNEW(stdString,(
		    "transform: openvdb resampleToMatch failed")));
	}
	}, vdwhy) )
