/*
 * ptaRandGaussianPt3D — rand_gaussian(center, sigma, n, seed) の **軸 3 本** の行
 *   (`rand_gaussian#pt3d`) の計算本体 (#3576 ・ 2026-09-22)。
 *   中心 c を **各軸の標準偏差 sigma** の等方ガウスで囲んだ点を n 個作り、pt-cloud3d で返す。
 *
 * ★★★ **シードは省略できない** — 同じ引数なら必ず同じ点群になる (ptaRand.cpp 冒頭)。
 * ★★ 行は **第 1 引数の要素数** (3 要素の配列) が選ぶ (#3572 の rand と同じ仕掛け)。
...
#include	"_ts2/c++/ptaRandGaussianPt3D_.h"

CLASS_TINYSTATE(pt/c++/ptaRandGaussianPt3D,pig/c++/ptsCalcBody)
...
ptaRandGaussianPt3D_::ptaRandGaussianPt3D_(TS_ARGS0)
...
ptaRandGaussianPt3D_::compute()
{
	...
	cloud = pt_cloud_random_gauss((*args)[0], (*args)[1], (*args)[2], (*args)[3],
	                        3, "rand_gaussian", err, (int)sizeof err);
	if ( ! cloud.is_notNull() )
...
sPtr<pigData>
ptaRandGaussianPt3D_::get_result()
{
