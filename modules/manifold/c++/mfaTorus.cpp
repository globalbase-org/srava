/*
 * mfaTorus — torus(R, r, seg) — トーラス (原点中心・軸 +Z) の計算本体 (manifold 版・#3474)。
...
#include	"_ts2/c++/mfaTorus_.h"

CLASS_TINYSTATE(mf/c++/mfaTorus,pig/c++/ptsCalcBody)
...
mfaTorus_::compute()
{
	...
	if ( srava_geo::check_segs(seg_in, 0, &seg) != srava_geo::SEGS_OK ) {
		result = mfa_err(thNEW(stdString,(srava_geo::segs_error("torus").c_str()))); return;
	}
	if ( !(R > 0) ) { result = mfa_err(thNEW(stdString,("torus: R (distance from the axis to the tube center) must be > 0"))); return; }
	if ( !(r > 0) ) { result = mfa_err(thNEW(stdString,("torus: r (tube radius) must be > 0"))); return; }
	if ( !(r < R) ) { result = mfa_err(thNEW(stdString,("torus: r must be < R (self-intersecting otherwise)"))); return; }
