...
mfaIntersection_::compute()
{
	...
	if ( a3.is_notNull() && b3.is_notNull() )      geom = a3->op_intersection(b3);   /* 3D */
	else if ( a2.is_notNull() && b2.is_notNull() ) geom = a2->op_intersection(b2);   /* 2D */
	else {
		result = thNEW(pigDataError,(thNEW(stdString,("intersection: needs two meshes"))));
		return;
	}
	if ( ! geom.is_notNull() )
		result = thNEW(pigDataError,(thNEW(stdString,("intersection: boolean failed"))));
}
