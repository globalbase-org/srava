/*
 * mfaCone — cone(r, h, seg) — 円錐 (原点中心・軸 +Z・底面 z=-h/2・頂点 z=+h/2) の計算本体 (manifold 版・#3474)。
...
#include	"_ts2/c++/mfaCone_.h"

CLASS_TINYSTATE(mf/c++/mfaCone,pig/c++/ptsCalcBody)
...
mfaCone_::compute()
{
	...
	if ( srava_geo::check_segs(seg_in, 0, &seg) != srava_geo::SEGS_OK ) {
		result = mfa_err(thNEW(stdString,(srava_geo::segs_error("cone").c_str()))); return;
	}
	if ( !(r > 0) ) { result = mfa_err(thNEW(stdString,("cone: radius must be > 0"))); return; }
	if ( !(h > 0) ) { result = mfa_err(thNEW(stdString,("cone: height must be > 0"))); return; }
