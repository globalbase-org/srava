...
### ★ 原則 — 使う `.so` を `module()` で名指し、候補列を `use` で書く {#name-the-modules}

**実際に使うモジュールに絞るのが原則である。** 同梱モジュールは増え続けており、この先も増える。
`include "module/all.sra";` のような一式ロードは、そのぶん常駐と起動の負荷が増え続ける。

それ以上に問題なのは**配線**である。候補列を書かずに一式を載せると、その op がどの `.so` に
当たるかは**ロード順と `priority` 任せ**になり、読んだだけでは決まらない。つまり
**書いていないモジュールへ黙って配線され、そのまま値が返る**。値が返るので気づく手掛かりが無い。

名指しと候補列は、この 2 つを同時に解く。**常駐が小さくなり、配線が読んで分かる**。
実際に、全ロードから 2 本の名指しへ移して**出力がバイト単位で変わらないまま常駐が目に見えて
減った**利用例がある（外れた十数本は、その用途では最初から使われていなかった）。

#### ⚠ `all.sra` は**禁じ手ではない** — 移行中・探索中の便宜品である {#all-sra-is-ok-while-migrating}

原則は「絞る」だが、**絞り切るまでの間**や、**どの `.so` が要るのかをこれから見極める段階**では
`include "module/all.sra";` を使ってよい。実際、`all.sra` と `use` を**併用**して
「ロードは一式・候補列は明示」という中間の形で運用している利用者がいる。

★ **移行そのものが割に合わないなら、しないという判断も妥当である。** 既存のスクリプトが多いほど
書き換えと再検証の費用が効くので、「得られる常駐の削減 < 払う移行の費用」なら見送ってよい。
判断の材料になるのは**自分の用途での実測**であって、この原則ではない。

⇒ 見極めがついたら `module()` の列へ置き換える。**落ちた op を 1 本ずつ足せば、それが実際の依存**
になる（[落ち方](#use-replaces)はエラー文で分かる）。
...
### `use` は候補列を**置き換える** — それが狙いである {#use-replaces}

`use [ … ]` を書いた瞬間、**そこに並べたものだけ**が候補になる。`module/all.sra` で
ロード済みでも、候補列に居なければ選ばれない。

★ **規約の全文は[言語リファレンス §`use 式` と予約変数 `USE_MODULES`](srava_language_reference.html#use-modules)**
にある（`use` の効く範囲・`apply` での引き継ぎ・`mod_only` / `mod_only_names` は そちら）。
ライブラリを書く人は[§ライブラリ関数は自分の候補列を関数の頭で宣言する](srava_language_reference.html#lib-use-decl)も
併せて読むこと。

★ **これは絞り込みが効いている証拠であって、`use` が強すぎるのではない。**
候補列を書かない状態（ロード順と `priority` に任せる状態）が**弱すぎた**のである。
並べ忘れた op は落ちるが、落ちれば直せる。黙って別のモジュールへ配線された答えは直せない。
...
★ 落ちるたびに、**そのスクリプトが本当に使っているモジュールが 1 本ずつ分かる**。
書き終えた候補列は、その一覧そのものになる。以後は読めば配線が分かり、
モジュールが増えても**自分の配線は動かない**。

⚠ 最初に踏むのは、**生成 op と、それ以外の op（I/O・ソルバ）が同じスクリプトに同居している**
ときである。粒度の既定値を入れたいのは生成 op だけでも、`use` は**スクリプト全体**に効くので、
I/O やソルバのモジュールも併せて列に並べる。
...
★ **これも狙いどおりである。** 候補列を書かなければ、この式は**書いていないモジュールへ
黙って渡り**、そこでキャストされて答えを返す。値が返るので気づく手掛かりが無い。
絞ると止まるのは、その「黙って」を消した結果である。
候補列が明示されていれば、外したものが効いていなかったことを**利用者自身が検算できる**。

★ 払うのは「**op を足すたびに候補列を見直す**」ぶんだけ。静かに壊れることはなく、
必ず上の形で落ちるので、気づけないコストにはならない。
