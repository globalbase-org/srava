...
struct srava_module_descriptor {
    ...
    void (*configure)(sPtr<pigData> opts);  // v10・。module(so,{opts}) のハッシュ全体を受け取る。
...
- 別モジュールが作った mesh を受理したい場合は、その型を `sig` の入力側に**明示列挙**する（fold 形なら
  ...
  ★ **`(N!)` = 主型で振り分けるが木に分解しない**。「主型で振り分ける」と「分解してよい」は
  独立した性質で、`hull` のように *入力から頂点しか使わない* op は分解しても損なうえ、
  退化した群で落ちる（詳細は内部設計資料「sig 文法設計」§3.3 — 公開ドキュメントには含めていない）。
  ⚠ bracket に書いてよいのは「主型と、**その op がこのスロットで受ける型**」だけ。ついでに foreign 型を
  足すと複数モジュールがマッチして**決着の根拠が「列挙の有無」から priority へ静かに移る**。
  ★ 以前は「主型へ**昇格して読める**型だけ」と書いていたが、これは**ブールを念頭に置いた言い方**
  だった。`hull` のように昇格ではなく「点を収穫する」op もあり、`pt-cloud3d` は
  `cg-mesh3d` へ昇格しない。主型は**その行が計算を行う表現**で、「最低 1 個」は
  **変換だけの仕事は引き受けない**という規則。

### 3.2.5 ★ 同じ op に複数の行を書く — `op#変種`（2026-09-19）

op 名は**キー**だが、**1 op = 1 行とは限らない**。`op#変種` という行を書くと、routing は
基底名 `op` で引くときに `op` と `op#…` を**前方一致で集め、OPS に書かれた順に先勝ち**で選ぶ。

```c
static const pigOpEntry OPS[] = {
  { "transform#xy", …, "(cg-cross2d)->cg-cross2d" },   // ★ 変種を先に
  { "transform",    …, "(cg-cross2d)->cg-face3d"  },   //   無条件は後に
};
```

* 行ごとに `sig` / `wiring`（計算本体）/ `in[]` / `nreq` / `commutative` を持てる
* 勝った**行名がそのまま agent へ渡る**（`C_OP`）。`lookup_op` は完全一致で当たる
* op 名は結果ハッシュに混ざるので **行ごとにキャッシュキーが分かれる**（`cache_version` を
  上げる必要はない）
* **基底行が無くてもよい** — `op#変種` だけを持つモジュールが書ける
* 行数に上限は無い

⚠ **無条件の行（`#` 無し）を変種より前に置くとロードが拒否される**（§3.3）。無条件の行は
常に成立するので、前に置くと変種が永久に選ばれないため。
⚠ `#` は利用者が書ける文字ではない。式に `transform#xy` とは書けない（内部名）。

### 3.3 ロード時に弾かれる申告

記述子は dlopen 時に自己矛盾を検査され、引っかかるとそのモジュールは**ロードされない**（fail-fast）:

- `sig` が文法違反（`[a,b](1)` のように `N < 2`・`(N)` の欠落・可変部が末尾でない 等）。★ `(N!)` の `!` は分解禁止（下記）
...
- ★ **無条件の行（`#` 無し）が、同じ基底名の `op#変種` より前に置かれている**（§3.2.5）
...
module("geogram.so",  { arity: 8 });                  // N 項評価の上限 (後述)
...
「op が見つからない」と言われたら、まずこれを疑う。
★ 名指すのは**原則**であって、開発中の手間ではない — 名指さなければ、どの `.so` が答えるかは
ロード順と `priority` 任せになる（`include "module/all.sra";` は移行中・探索中の便宜品）。
...
// ★ grace_ms / panic_ms の選び方 。**未指定 = 0 = 安全側**なので書き忘れは事故にならないが、
...
//    register_hash_salt でグローバルへ書き込んでいたが (で撤去)、.so は「誰のレジストリか」を
