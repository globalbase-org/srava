...
**ボクセル化の規約（v1.1.0-rc2 以降）** — 基準値を持つ側が知っておくべき点:

- サンプル点は**セル中心** `origin + (i+1/2)·dx`。**軸はセル境界に来る**ので、半領域の index0 を
  `r = 0.5·dy` と解釈する軸対称ソルバの規約と一致する（従来あった半セルのずれが無い）。
- 内外判定は厳密で、面・辺・頂点にサンプルがちょうど載る縮退も一貫して解く。**同じ形状なら常に同じ
  マスク**が出る（実行環境・実行回に依存しない）。z 方向の充填は半開区間なので、接する立体が
  隙間なく・二重なく並ぶ。
- ⚠ **rc1 以前に作った `vox.h5` とは voxel 数が変わる**。旧版は縮退時の丸めが不定で、同一入力でも
  実行ごとに数十〜数百 voxel 揺れることがあった。基準値・凍結データを持っている場合は再取得が必要。
...
- srava: `export_vox` は **`openvdb_cg.so` モジュールが提供**（2026-09-01・ で `cgal.so` から移設）。
  ...
  `include "module/all.sra";` を使っているなら追加は不要（一覧に含めてある）。★ ただし**原則は名指し**なので、
  この op が要ると分かった時点で `module("openvdb_cg.so",{})` を書く方がよい。
