...
2D パスを渡すと明示エラーになる。★ `openvdb.so` は `tube_ruled(path, segs, dx)`（`dx` も必須）。
...
- 関連: `tube`（occt の B-spline 版）, `ribbon2d`, `loft_ruled`
...
```
- 関連: `tube_ruled`, `ribbon2d`, `bezier`, `spline`
...
`6 + 10d + πd²`・内側は角が立つ）。**面に厚みを付けて立体にする**のは別 op
